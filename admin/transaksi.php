...
<body>
										...
										<div class="form-group col-lg-6">
											<label>Judul Buku</label>
												<select class="form-control" name="id_buku" onchange="changeValue(this.value)">
													...
													<?php
													$queryJudul = "SELECT b.id, b.judul, b.pengarang, b.harga, p.nama_penerbit 
																	FROM buku b 
														...
														<tr>
															...
															<td style="width: 70px; text-align:center;">
																<div class="input-group">
																	<input type="number" class="form-control qty-reduce" style="width: 60px;" min="1" value="1">
																	<span class="input-group-btn">
																		<a id="kurang" class="btn btn-danger reduce-btn" href="proses_kurang_beli.php?id=<?php echo $baris['id']; ?>&no_transaksi=<?php echo $noTransaksi; ?>&qty=" onclick="return confirmReduce(this, '<?php echo $baris['dataBuku']; ?>')"><i class="fa fa-minus"></i></a>
																	</span>
																</div>
															</td>
															</td>
		...
		$(document).ready(function() {
			$("#tambah").click(function() {
				...
				$.ajax({
					...
					data: data,
					success: function() {
						location.reload(); // Refresh halaman setelah menambah item
					}
				});
			});
	...
	<script>
    function confirmReduce(element, bookTitle) {
        var qtyToReduce = element.parentNode.parentNode.querySelector('.qty-reduce').value;
        var confirmMessage = 'Anda yakin ingin mengurangi ' + qtyToReduce + ' buku ' + bookTitle + '?';
        
        if (confirm(confirmMessage)) {
            // Modify the href to include the qty
            element.href += qtyToReduce;
            return true;
        }
        return false;
    }
